#!/usr/bin/env python3
"""Generate Riale push spritesheets (8 directions) from 3 sources + substitution + mirroring."""

from PIL import Image
import numpy as np
import os

SRC = "/home/yipene/Documents/Projects/artworks/Maleck/Riale/anim/push"
DST = "/home/yipene/Documents/Projects/artworks/Maleck/game/art/riale/push"
TARGET_FEET_Y = 344
CELL_W = 768
CELL_H = 448
ROWS = 8
...
# Only 3 source directions: use cardinals for missing diagonals
SOURCES = {
    "down":       "riale_push_down.png",
    "right":      "riale_push_right.png",
    "up":         "riale_push_up.png",
}

# Missing diagonals use the nearest cardinal
FALLBACK = {
    "down_right": "down",
    "up_right":   "up",
}
...
def process_direction(dir_name, img):
    ...
    dst_path = os.path.join(DST, f"{dir_name}_push.png")
...
def main():
    ...
    # Fallback for missing diagonals
    for dst_name, src_name in FALLBACK.items():
        if src_name in loaded:
            print(f"  Fallback {src_name:15s} -> {dst_name} (missing source)")
            process_direction(dst_name, loaded[src_name])

    # Mirrors
    for src_name, dst_name in MIRRORS:
        if src_name in loaded or src_name in FALLBACK:
            actual_src = FALLBACK.get(src_name, src_name)
            if actual_src in loaded:
                print(f"  Mirror {src_name:15s} -> {dst_name}")
                process_direction(dst_name, mirror_img(loaded[actual_src]))

    for d in ["down", "down_left", "left", "up_left", "up", "up_right", "right", "down_right"]:
        p = os.path.join(DST, f"{d}_push.png")
        if os.path.exists(p):
