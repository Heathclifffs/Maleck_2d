#!/usr/bin/env python3
"""Generate Riale attack1 spritesheets (8 directions) from 5 sources + mirroring."""

from PIL import Image
import numpy as np
import os

SRC = "/home/yipene/Documents/Projects/artworks/Maleck/Riale/anim/combat/attack1"
DST = "/home/yipene/Documents/Projects/artworks/Maleck/game/art/riale/attack1"
TARGET_FEET_Y = 344
CELL_W = 768
CELL_H = 448
ROWS = 8
...
SOURCES = {
    "down":       "riale_attack_1_down.png",
    "down_right": "riale_attack_1_down_right.png",
    "up":         "riale_attack_1_up.png",
    "up_right":   "riale_attack_1_up_right.png",
}

# The "left" source file actually shows character facing RIGHT
LEFT_SRC_AS_RIGHT = "riale_attack_1_left.png"

MIRRORS = [("down_right", "down_left"), ("up_right", "up_left")]
...
def process_direction(dir_name, img):
    ...
    dst_path = os.path.join(DST, f"{dir_name}_attack1.png")
...
def main():
    ...
    # Process cardinal+diag sources
    for d in ["down", "down_right", "up", "up_right"]:
        if d in loaded:
            process_direction(d, loaded[d])

    # Left source is actually right-facing → process as "right", mirror to "left"
    left_src_path = os.path.join(SRC, LEFT_SRC_AS_RIGHT)
    if os.path.exists(left_src_path):
        right_img = Image.open(left_src_path).convert("RGBA")
        print(f"Loaded left-as-right from {LEFT_SRC_AS_RIGHT}")
        process_direction("right", right_img)
        print(f"  Mirror (right) -> left")
        process_direction("left", mirror_img(right_img))
    ...
    for d in ["down", "down_left", "left", "up_left", "up", "up_right", "right", "down_right"]:
        p = os.path.join(DST, f"{d}_attack1.png")
        if os.path.exists(p):
